...
#----bring in covariates
setwd("C:/Users/cjbsu/OneDrive/UConn/Research/Dissertation/Papers/Trout movement/")
covs<-read.csv("covs_2024.csv",header = TRUE)
covs <- covs[,c(5:12)]
...
ms <- jags(data=bugs.data, inits = inits, parameters.to.save = parameters, 
           model.file="statespace.txt", n.adapt= na, n.chains=nc, n.thin=nt, n.iter=ni, n.burnin=nb, parallel=TRUE)

print(ms, digits = 3)
ms_output_pred <- as.data.frame(jags.View(ms)) 
write.csv(ms_output_pred, "ms_output_pred_1hr_simulated_data.2024.csv")